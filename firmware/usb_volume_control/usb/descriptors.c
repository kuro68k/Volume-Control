...
void generate_serial(void)
{
	...
	serial_string->bLength = sizeof(USB_StringDescriptor_t) + (USB_SERIAL_NUMBER_LENGTH_CHARS*2);

	__CHAR16_TYPE__ *c = (__CHAR16_TYPE__ *)&serial_string->bString;
	register8_t *ptr = &SIGROW.SERNUM0;
...
_Static_assert(((USB_SERIAL_NUMBER_LENGTH_CHARS*2)+2) <= USB_EP0_BUFFER_SIZE, "Serial number string exceeds EP0 buffer size");
#endif
...
void handle_msft_compatible(void)
{
	...
	if (usb_setup.wIndex == 0x0005) {
		address = (void*)&msft_extended;
		size = msft_extended.dwLength;
	} else
#endif
	if (usb_setup.wIndex == 0x0004) {
		address = (void*)&msft_compatible;
	...
	switch (type)
	{
		...
		case USB_DTYPE_String:
			switch (index)
			{
				...
				case 0x03:
					generate_serial();
					return sizeof(USB_StringDescriptor_t) + (USB_SERIAL_NUMBER_LENGTH_CHARS*2);
#endif
#ifdef USB_DFU_RUNTIME
				case 0x10:
			...
			size = pgm_read_byte(address + offsetof(USB_StringDescriptor_t, bLength));
			break;
	}
